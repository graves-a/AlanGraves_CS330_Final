...
namespace
{
    ...
    struct GLMesh
    {
        GLuint tissueBoxVAO, planeVAO, tissueVAO;         // Handle for the vertex array object
        GLuint tissueBoxVBO, planeVBO, tissueVBO;         // Handle for the vertex buffer object
        GLuint nTissueBoxVertices, nPlaneVertices, nTissueVertices;    // Number of indices of the mesh
    };
...
int main(int argc, char* argv[])
{
    ...
    if (!UCreateTexture(tissueBoxFile, gTissueBoxTextureId))
    {
        cout << "Failed to load texture " << tissueBoxFile << endl;
        return EXIT_FAILURE;
    }
    ...
    const char* planeFile = "../../resources/textures/leather.jpg";
    if (!UCreateTexture(planeFile, gPlaneTextureId))
    {
        cout << "Failed to load texture " << planeFile << endl;
        return EXIT_FAILURE;
    }
    glUseProgram(gCubeProgramId);
    glUniform1i(glGetUniformLocation(gCubeProgramId, "uPlaneTexture"), 0);
    const char* tissueFile = "../../resources/textures/brick.jpg";
    if (!UCreateTexture(planeFile, gTissueTextureId))
    {
        cout << "Failed to load texture " << planeFile << endl;
        return EXIT_FAILURE;
    }
    ...
    glUniform1i(glGetUniformLocation(gCubeProgramId, "uTissueTexture"), 0);
...
void URender()
{
    ...
    // Draws the triangles
    glDrawArrays(GL_TRIANGLES, 0, gMesh.nTissueBoxVertices);
    ...
    glDrawArrays(GL_TRIANGLES, 0, gMesh.nTissueVertices);
...
void UCreateMesh(GLMesh& mesh)
{
   ...
   -0.5f,  0.5f, -0.5f,  0.0f,  1.0f,  0.0f,  0.0f, 1.0f
    ...
    mesh.nTissueBoxVertices = sizeof(tissueBoxV) / (sizeof(tissueBoxV[0]) * (floatsPerVertex + floatsPerNormal + floatsPerUV));
    mesh.nPlaneVertices = sizeof(planeV) / (sizeof(planeV[0]) * (floatsPerVertex + floatsPerNormal + floatsPerUV));
    mesh.nTissueVertices = sizeof(tissueV) / (sizeof(tissueV[0]) * (floatsPerVertex + floatsPerNormal + floatsPerUV));

    glGenVertexArrays(1, &mesh.tissueBoxVAO); // we can also generate multiple VAOs or buffers at the same time
    glBindVertexArray(mesh.tissueBoxVAO);

    // Create 2 buffers: first one for the vertex data; second one for the indices
    glGenBuffers(1, &mesh.tissueBoxVBO);
    glBindBuffer(GL_ARRAY_BUFFER, mesh.tissueBoxVBO); // Activates the buffer
    glBufferData(GL_ARRAY_BUFFER, sizeof(tissueBoxV), tissueBoxV, GL_STATIC_DRAW); // Sends vertex or coordinate data to the GPU
    ...
    //glGenVertexArrays(1, &mesh.tissueVAO); // we can also generate multiple VAOs or buffers at the same time
    ...
    //glBindBuffer(GL_ARRAY_BUFFER, mesh.planeVBO);
    //// Activates the buffer
    //glBufferData(GL_ARRAY_BUFFER, sizeof(tissueV), tissueV, GL_STATIC_DRAW); // Sends vertex or coordinate data to the GPU
...
void UDestroyMesh(GLMesh& mesh)
{
    ...
    glDeleteVertexArrays(1, &mesh.tissueVAO);
    glDeleteBuffers(1, &mesh.tissueVBO);
